...
| 4 | **A.8.25 / A.8.29 (supply chain)** | `govulncheck` covers known-vuln detection and CI now produces an SBOM (`syft`, SPDX + CycloneDX — `.github/workflows/ci.yml`, `security` job). No automated dependency-update bot is wired up yet. | Optional hardening: adopt a dependency-update bot; attach the generated SBOM to releases. |
