...
export function RunHistory({ tenant, workspace, graphID, currentRunID, onSelect, }) {
    ...
    return (_jsxs("div", { ref: popRef, style: { position: "relative" }, children: [_jsxs("button", { type: "button", className: "ghost", onClick: () => setOpen((v) => !v), style: { display: "inline-flex", alignItems: "center", gap: 6 }, children: [_jsx(History, { size: 14 }), currentRunID ? (_jsxs(_Fragment, { children: [currentStatus && (_jsx("span", { className: "status-dot " + currentStatus })), _jsx("span", { style: { fontFamily: "var(--font-mono)", fontSize: 12 }, children: currentRunID.slice(0, 8) })] })) : (_jsx("span", { style: { fontSize: 12, color: "var(--muted)" }, children: "No run" })), _jsx(ChevronDown, { size: 12 })] }), open && (_jsxs("div", { className: "run-history-pop", children: [_jsx("div", { className: "run-history-head", children: "Recent runs" }), _jsx("div", { className: "run-history-filters", children: STATUS_CHIPS.map((c) => (_jsx("button", { type: "button", className: "run-filter-chip" + (filter === c.value ? " active" : ""), onClick: () => setFilter(c.value), children: c.label }, c.label))) }), loading && runs.length === 0 && (_jsx("div", { className: "run-history-empty", children: "Loading\u2026" })), !loading && runs.length === 0 && (_jsx("div", { className: "run-history-empty", children: "No runs yet." })), runs.map((r) => (_jsxs("button", { type: "button", className: "run-history-row" + (r.id === currentRunID ? " active" : ""), onClick: () => {
                            onSelect(r.id);
                            setOpen(false);
