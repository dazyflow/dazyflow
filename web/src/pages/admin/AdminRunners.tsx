            ...
            <table className="run-table">
              ...
              <tbody>
                {runners.map((r) => (
                  <tr
                    key={r.name}
                    className={confirmRemove === r.name ? "row-confirming" : undefined}
                  >
                    {/* The name opens the machine's settings, where its tags
                        are. Clicking a machine is how anyone expects to get at
                        it, and the whole cell is the hit area — see
                        .run-table td.run-name-cell. */}
                    <td className="run-name-cell">
                      <Link to={`/admin/runners/${encodeURIComponent(r.name)}`}>{r.name}</Link>
                    </td>
