...
export const SV_DESCRIPTIONS: DescriptionMap = {
  ...
  code: {
    en: "091706d4",
    sv: "Skriv lite JavaScript när de färdiga stegen inte kan säga det du menar. Värdet som kopplas in på 'in' kommer fram som `input`, och det du returnerar med `return` går ut på 'out' — ett objekt, en lista, ett tal, en text, ett ja/nej. `console.log(…)` skriver till stegets konsol medan du tittar, och står kvar när körningen är över — varje rad märkt med nivån du skrev den på, fel i rött och varningar i gult, och den rad i skriptet som skrev den. Samma rader går ut på Loggrader, så ett flöde kan mejla dem eller spara dem.\n\nVälj hur det körs. 'En gång, över allt' ger dig hela indatat på en gång — ta det för att forma om ett svar, räkna över en lista eller bygga ett värde som inget annat steg kan. 'En gång per rad' kör koden en gång per rad i stället, med raden som `row` och dess position som `index`, och samlar det du returnerar i en ny lista; returnerar du ingenting för en rad faller den raden bort, vilket gör det här till både filter och omvandling. Varje rad körs i samma sandlåda, så ett värde du lämnar på en global överlever raden som satte det — det är så du bär med dig en löpande summa från en rad till nästa.\n\nDet körs inne i Dazyflow utan väg ut: inget nätverk, inga filer, inga bibliotek att importera. Det är med avsikt — anropa API:er med steget Webbanrop och läs filer med filstegen, och koppla sedan in resultatet här. Det finns ingen klocka att vänta på heller: inga timers, och inga promises eller async/await, så gör arbetet i ordning och returnera värdet självt i stället för ett löfte om ett. `Intl` saknas också, vilket betyder att `toLocaleString('sv-SE')` kastar fel i stället för att formatera — skriv formatet du vill ha, eller låt steget Datum och tid göra det. Skript får några sekunder som standard (höj Tidsgräns till som mest 30 sekunder), 8 MB returnerad data och de första 200 konsolraderna. Behöver du en riktig körmiljö, ett bibliotek eller ett nätverk använder du Kör på din maskin i stället.\n\nFör ett enda värde — lite räkning, ett fält som ska plockas ut — är steget Uttryck lättare, och Lägg till en beräknad kolumn gör samma sak per rad helt utan kod.",
  },
  compare: {
    en: "e6392d92",
  ...
  date: {
    en: "a5960863",
    sv: 'Arbeta med datum och tid: läs aktuell tid, tolka en tidsstämpel som kommit in som text, flytta den med en förskjutning, lås den till en tid på dygnet, växla till en tidszon och skriv ut den i det format du vill. Koppla in ett värde i \'in\' (en ISO 8601-sträng, en Unix-tidsstämpel eller vanlig datumtext) eller lämna porten okopplad för att använda aktuell tid. Stegen tillämpas i den ordningen, och det är det som gör att de går att kombinera: \'add\' flyttar med "3d", "-2h30m" eller "1w" — så "1d" blir i morgon — och \'at\' sätter sedan klockan ("09:00"), vilket är det som gör "24 timmar från nu" till "i morgon bitti". \'tz\' väljer vilken tidszon utdatat skrivs i — sök vilken IANA-zon som helst — och den avgör vilken dag och vilken timme det blir, inte bara hur förskjutningen märks. \'locale\' skriver namn på veckodagar och månader på ett språk — engelska eller svenska, och följer flödets eget språk om inte steget säger annat (ISO-, Unix- och e-postformaten förblir engelska eftersom de är maskinformat). \'format\' väljer ett namngivet format — datum, datum och tid, 12- eller 24-timmarsklocka, veckodagens namn ("Thursday" / "torsdag"), Unix, e-post/HTTP — eller Custom för ett eget som du skriver med tokens som YYYY MM DD HH mm ss ("DD/MM/YYYY", "ddd D MMM"), med bokstavlig text inom hakparenteser ("[week of] D MMM"). Skickar ut den formaterade strängen på \'out\' och de uppdelade delarna (år, månad, veckodag, …) på \'value\'; lägg in det i vilken text som helst med ${upstream.<step>.out}.',
  },
  dedupe_rows: {
    en: "9c4a95e1",
  ...
  gcal_on_event_change: {
    en: "32bb065b",
    sv: 'Bevakar en Google-kalender och startar flödet när något i den ändras — en händelse som bokas, flyttas, byter namn eller avbokas. De ändrade händelserna kommer ut som en lista, redo för För varje, och var och en bär `change`: nybokad, ändrad eller avbokad. En avbokad händelse har fortfarande sina detaljer med sig, så ett flöde kan berätta vad som ställdes in.\n\nDen första kontrollen efter att du publicerat noterar var kalendern står och utlöser ingenting, så att slå på bevakningen spelar inte upp kalenderns historik. Därefter ser en kontroll bara det som ändrats sedan kontrollen före den.\n\nÅterkommande händelser rapporteras som serien, inte som utskrivna tillfällen: att redigera en veckovis standup utlöser en gång, inte en gång för varje vecka den någonsin kommer att gå. Använd "Innan en händelse börjar" när du vill ha de enskilda bokningarna.\n\nGoogles kalendernotiser bär inga egna detaljer och behöver en publikt verifierad callback för att komma fram alls, så det här pollar i stället — ett API-anrop per kontroll, vare sig något ändrats eller inte.',
  },
  gcal_on_event_start: {
    en: "e7fd45e1",
    sv: 'Startar flödet en bestämd tid innan en händelse i en Google-kalender börjar — tio minuter före ett möte, en timme före ett leveransfönster, kvällen före en bokning. Händelserna som snart börjar kommer ut som en lista, redo för För varje, var och en med `minutes_until_start` jämte titel, tid, plats och deltagare.\n\nVarje bokning aviseras en gång. En som flyttas aviseras igen för sin nya tid, för påminnelsen som är värd något är den för när saken faktiskt händer. Avbokade händelser lämnas i fred.\n\nHåll kontrollintervallet inte längre än varselet du ber om, annars kan en händelse nås efter att den redan börjat. En kontroll som aldrig blev av tas inte igen heller: en händelse vars start har glidit mer än en kvart bakåt lämnas i fred i stället för att aviseras sent, så en daemon som legat nere över natten vaknar inte och larmar om gårdagens möten.\n\nHeldagshändelser har ingen klocktid att räkna bakåt från, så de lämnas utanför om du inte ber om dem; ombedda räknas de från midnatt i den tidszon du anger.\n\nTill skillnad från "När en händelse ändras" utlöser den första kontrollen efter publicering — den når det som redan ligger inne i varselfönstret, oftast nästa händelse eller två. Alternativet är att publicera vid nio och tyst missa mötet halv tio.',
  },
  gemini: {
    en: "76d1996e",
    sv: "Skicka en prompt och få ett svar tillbaka — sammanfatta text från ett tidigare steg, klassificera ett indata eller generera text. Koppla in texten som ska bearbetas i ingången Prompt, eller skriv en prompt direkt på steget.",
  ...
  join_rows: {
    en: "6cf314fa",
    sv: 'Parar ihop två listor med rader på en kolumn de har gemensam — uppslagssteget. Ange vilken kolumn i första listan som hör samman med vilken kolumn i den andra (en orders customer_id med en kunds id, till exempel), och varje par kommer ut som en rad som bär kolumnerna från båda sidor.\n\nDu väljer vad som kommer ut: bara raderna som matchade, varje rad från en av listorna oavsett om den matchade eller inte, allt från båda — eller bara raderna ur första listan som INTE har någon träff i den andra. Det sista svarar på frågan "vilka av de här har jag inte tagit hand om än?": lägg dagens rader på den första ingången och det du redan har sparat på den andra, och ut kommer bara de nya.\n\nTvå detaljer värda att känna till. Om båda listorna har en kolumn med samma namn får den andra listans kopia "_right" tillagt så att ingen av dem tappas tyst (ändra ändelsen på steget). Och när en rad i första listan matchar flera i den andra får du en utdatarad per träff.',
  },
  json: {
    en: "067c3bd7",
  ...
  knowledge_forget: {
    en: "4f4f5a1e",
    sv: 'Ta ut ett dokument ur en kunskapsbas igen. Läggs en källa till på nytt ersätts dess avsnitt, så det här är för det andra fallet — sidan som inte längre finns, policyn som dragits tillbaka, kunden som bad om att bli glömd.\n\nAnge samma "Varifrån det kom" som dokumentet lades till under. Varje avsnitt som kom därifrån försvinner; resten av basen är orörd.\n\nAtt tömma en hel bas är med flit svårare: lämnar du källan tom vägrar steget om du inte också slår på "Glöm hela basen". Det frigör namnet så att basen kan byggas på nytt — med en annan inbäddningsmodell, om det är därför du rensar den.\n\nDet behöver ingen anslutning: att glömma läser inga inbäddningar, så det fungerar även innan Kunskap är uppsatt.',
  },
  lookup: {
    en: "95a7c663",
  ...
  render_table: {
    en: "bfb00eec",
    sv: 'Gör en radlista direkt till en färdig HTML-tabell — kolumnnamnen blir rubrikrad och varje rad blir en tabellrad. Till skillnad från Gör text finns ingen mall att skriva och inga kolumnnamn att skriva in: rubrikerna kommer från de kolumner datat faktiskt har, så de kan inte glida ifrån källan (inget "no such key" vid körning). Koppla in en radlista i `rows` och utgången `html` till ett meddelandesteg — t.ex. Innehåll i ett Skicka e-post-steg. Rubrikerna är datats egna kolumnnamn om du inte byter namn på dem. `column_labels` är en enkel {kolumn: rubrik}-karta — {"customer_email":"Kund"} sätter rubriken "Kund" på den kolumnen och lämnar alla andra kolumner i fred, så att byta en rubrik betyder inte att du måste räkna upp allihop. `columns` kan också bära ett `label` per post, som vinner över kartan för den kolumnen. Sätt `title` för att namnge tabellen — namnet visas som rubrik ovanför rubrikraden, och en ${upstream.…}-referens fungerar där, så namnet kan bära körningens data ("Ordrar för 2026-08-26"). Med noll rader skickas `empty` ut (standard ""), så ett tomt resultat ger den reserv du valt i stället för en tom tabell.',
  },
  render_template: {
    en: "a3070120",
  ...
  route_rows: {
    en: "7c533130",
    sv: 'Skickar varje rad olika väg beroende på vad som står i den — utgreningssteget. Du ger den en ordnad lista med regler, var och en ett villkor plus rutten den matar: för varje rad vinner den FÖRSTA regel den uppfyller, och raden lämnar steget på den ruttens utgång. En rad som inte uppfyller någon regel lämnar det på Allt annat.\n\nAnvänd det för att dela en lista i vägar per kategori — svenska, norska och brittiska ordrar var och en till sina egna senare steg, eller de ordrar med högt värde en annan väg än resten. Det finns åtta rutter plus Allt annat; behöver du fler, mata Allt annat in i ett andra Dirigera rader-steg.\n\nEftersom den första träffen vinner ska du ordna reglerna från mest till minst specifik. En regel för "poäng 90 eller mer" måste komma FÖRE "poäng 50 eller mer" — i omvänd ordning sväljer den lösare regeln de rader den strikta var tänkt att fånga.',
  },
  rss: {
    en: "dbfb8c10",
  ...
  run_on_runner: {
    en: "a640bcf6",
    sv: "Kör ett skript på en av organisationens egna maskiner — en server, en laptop, vad som helst som kör Dazyflows körnodsagent. Använd det när arbetet behöver ett bibliotek, ett verktyg eller ett nät som de inbyggda stegen inte når. Ange VAR med etiketter: jobbet går till en maskin som har varje etikett du listar, så 'linux' betyder vilken som helst av dina linux-maskiner och 'linux + gpu' en som är båda. Varje maskins eget namn är också en etikett, så att lista ett namn låser steget till just den maskinen. Välj vad som startar skriptet — maskinens eget skal, sh, bash, Python, PowerShell eller Node — och skriv skriptet i rutan; eller koppla in det på ingången 'script' för att bygga det i ett tidigare steg. Skicka in värden som miljövariabler — ${secret.NAMN} för en inloggningsuppgift, som når maskinen utan att någonsin skrivas in i flödet. Värdet som kopplas in i 'in' kommer in på skriptets standard input; det skriptet skriver ut kommer tillbaka på 'out'. En avslutningskod som inte är noll gör att steget misslyckas, med skriptets felutskrift bifogad — eller sätt 'Om skriptet avslutas med en kod som inte är noll' till att fortsätta, och förgrena i stället på utgången Avslutningskod så att flödet hanterar sina egna fel.",
  },
  secret_set: {
    en: "0b7ba489",
  ...
  ssh_run: {
    en: "79b6c6b3",
    sv: "Kör ett kommando på en server över SSH och gå vidare med det den skrev ut. Välj server med namn — de ställs in en gång på sidan Servrar, så ingen adress, inget användarnamn och ingen nyckel skrivs någonsin in i ett flöde. Det kommandot skriver ut kommer tillbaka på 'out', felutskriften på Felutskrift, och koden den avslutades med på Avslutningskod.\n\nEn avslutningskod som inte är noll gör som standard att steget misslyckas, med kommandots felutskrift bifogad. Sätt 'Om kommandot avslutas med en kod som inte är noll' till att fortsätta i stället, och förgrena på Avslutningskod — så som avslutningskoder är tänkta att fungera, där 2 kan betyda ”inget att göra i dag” snarare än ”trasigt”.\n\nSkicka in värden som miljövariabler — ${secret.NAMN} för en inloggningsuppgift, som når servern utan att skrivas in i flödet eller i körningens logg. Värdet som kopplas in i 'in' kommer in på kommandots standard input. Koppla in Serveradress för att köra samma kommando över en hel maskinpark: den valda serverns användarnamn och nyckel återanvänds mot varje adress, så ge den inloggningen en known_hosts som täcker alla maskiner i stället för ett enda fingeravtryck.\n\nIngen terminal är kopplad, så ett kommando som stannar och frågar något väntar tills stegets tid tar slut — använd 'sudo -n' och en NOPASSWD-regel i stället för bara 'sudo'. Kommandot körs i kontots skal, så det skrivs som du skulle skriva det i ett POSIX-skal.",
  },
  sftp_delete_file: {
    en: "adcc1d2d",
    sv: "Ta bort en fil på en SFTP-server. Lägg det efter Hämta fil för att beta av en brevlåda så som de flesta motparter förväntar sig: ta filen och rensa den sedan, så att mappen är listan över det som fortfarande är kvar att göra.\n\nPeka ut en fil på samma sätt som Hämta fil gör — koppla ingången 'Fil' från en För varje över Lista filer så följer hela posten med, eller skriv en sökväg.\n\nEn mapp nekas i stället för att tömmas: det här tar bort en namngiven fil och inget annat. Om filen redan är borta misslyckas steget som standard, eftersom det oftast betyder att något annat arbetar i samma mapp; ställ 'Om den inte finns' på att fortsätta när steget körs efter något som kan ha tagit bort den redan.",
  },
  sftp_download_file: {
    en: "1f85822f",
  ...
  sftp_move_file: {
    en: "9c6890b7",
    sv: "Flytta en fil till en annan mapp på samma SFTP-server, eller byt namn på den där den ligger. Lägg det efter Hämta fil för att beta av en brevlåda så som de flesta motparter förväntar sig: ta filen och flytta den sedan till /processed, så att mappen är listan över det som är kvar att göra och båda sidor ser vad som hanterats.\n\nInget överförs — servern byter namn på filen på plats, så storleken spelar ingen roll och en halvflyttad fil är omöjlig.\n\nPeka ut en fil på samma sätt som Hämta fil gör: koppla ingången 'Fil' från en För varje över Lista filer så följer hela posten med, eller skriv en sökväg. Ger du 'Flytta till' en mapp behåller filen sitt namn; ger du en fullständig sökväg byter den namn samtidigt. Ett datum i målet — /processed/${date.today} — är det vanliga sättet att få ett arkiv värt att läsa.\n\nAtt flytta ovanpå en fil som redan finns misslyckas om du inte tillåter det, eftersom den andra filen på de flesta servrar helt enkelt går förlorad. Att flytta en fil som inte finns misslyckas också; ändra 'Om den inte finns' när steget körs efter något som kan ha flyttat den redan.",
  },
  sftp_on_new_file: {
    en: "4c4c479e",
    sv: 'Bevakar en mapp på en SFTP-server och startar flödet när en fil landar i den — en banks brevlåda, en leverantörs feed, en löneexport. Avgränsa med ett mönster som "*.csv" så kommer varje fil ut med namn, fullständig sökväg, storlek och ändringstid, äldst först, redo att loopa över med För varje och lämna till Hämta fil.\n\nVarje fil rapporteras en gång. Den första kontrollen efter att du publicerat noterar var mappen står och utlöser ingenting, så att slå på en bevakning betar inte av mappens hela historik. Därefter ser en kontroll bara det som kommit in sedan den förra — inklusive filerna som delar sekund med den nyaste, vilket är det som hindrar en feed som släpper tjugo filer 03:00:00 från att tappa eftersläntrarna.\n\nEn skur större än "Max antal filer" fördröjs i stället för att hoppas över: äldsta väntande fil går först, i ankomstordning, och resten följer vid nästa kontroller.\n\nInget flyttas eller raderas på köpet — filen ligger kvar, och det är vattenmärket som hindrar den från att komma runt igen.',
  },
  sftp_sync_folder: {
    en: "c0a89a23",
    sv: "Kopiera en hel mapp mellan en SFTP-server och din arbetsyta, och flytta bara filerna som skiljer sig. Allt som redan finns på båda sidor med samma storlek och tid lämnas i fred, så första körningen gör jobbet och de därefter är nästan gratis.\n\nVälj riktning: hämta ner en mapp från servern, eller skicka upp en till den. Undermappar följer med som standard, och ett mönster som \"*.csv\" avgränsar vad som räknas.\n\n'Filer' bär bara det den här körningen faktiskt flyttade, så ett steg efter den arbetar på ändringarna i stället för hela mappen — vilket är skillnaden mot att lista en mapp och loopa över allt i den.\n\nAtt ta bort det som inte längre finns på andra sidan är avstängt som standard. En spegling som tar bort är en riktig spegling, och det är också inställningen som kan tömma en mapp för att ett mönster var fel, så den måste efterfrågas.\n\nDet här är vad du ska ta till i stället för rsync: rsyncs knep block för block lönar sig på en stor fil som mestadels inte ändrats, och en feed som släpper en ny fil varje natt har inget åt det att spara.",
  },
  sftp_upload_file: {
    en: "1a00dd26",
  ...
  web_watch: {
    en: "ae498fe3",
    sv: "Håll ett öga på en webbsida och låt flödet köra bara när den faktiskt ändras — ett pris, en statussida, en upphandlingslista, en jobbannonssida. Kombinera med en Intervall-trigger. Första kontrollen registrerar tyst vad sidan säger idag; från och med då jämför varje kontroll. Steg som är kopplade till Vid ändring ligger vilande så länge inget ändras, så ett larm går bara ut när det finns något att säga. Som standard jämförs orden på sidan, inte HTML:en bakom dem, vilket hindrar osynliga ändringar i markup från att slå falskt alarm. Vill du bevaka ett enda tal i stället för hela sidan anger du ett mönster i 'Bevaka bara detta'. Det fungerar lika bra på ett API som på en sida: stäng av ordjämförelsen, lägg till de rubriker API:et behöver och låt mönstret plocka ut det enda fältet ur JSON:en. Ett undantag är värt att känna till: genomsläppsstiftet för vidare sitt värde vid varje kontroll, ändrad eller inte — koppla därför från Vid ändring när du menar \"bara när något ändrades\".",
  },
  reply: {
    en: "3f68eafb",
