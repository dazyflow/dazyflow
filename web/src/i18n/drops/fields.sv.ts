...
export const SV_FIELD_HELP: Record<string, string> = {
  ...
  "Join flavor. inner = matched only. left = all left + matched right. right = all right + matched left. outer = full union. anti = the left rows that have no match on the right — the \"which of these are new?\" question — emitted with their own columns only.": "Typ av join. inner = bara träffar. left = alla vänstra + matchade högra. right = alla högra + matchade vänstra. outer = full union. anti = de vänstra rader som inte har någon träff på högra sidan — frågan \"vilka av dessa är nya?\" — och de skickas ut med bara sina egna kolumner.",
  ...
  "Optional dot-path into the input to match on (e.g. status). Empty matches the whole input value. The full payload rides through regardless.": "Valfri punktnotationssökväg in i indatat att matcha på (t.ex. status). Tomt matchar hela indatavärdet. Hela nyttolasten går vidare oavsett.",
...
export const SV_ENUM_LABELS: Record<string, string> = {
  "All left rows": "Alla vänstra rader",
  "All right rows": "Alla högra rader",
  "English": "Engelska",
  "Everything from both": "Allt från båda",
  ...
  "Only left rows with NO match": "Bara vänstra rader UTAN träff",
  "Only matching rows": "Bara matchande rader",
