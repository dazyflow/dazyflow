...
//
// A guide page has two halves written in different places: the markdown in
// docs/guide/, and a row in NAV below. Miss the second and the page is
// unreachable — no build fails, no link 404s, the page simply isn't there. That
// is how members, roles and approvals went undocumented while being primary UI.
//
// These assert against docs/guide/ (committed) rather than web/src/docs/content/
// (generated by `make docs-content` and git-ignored). The few checks that need
// the rendered bundle skip when it hasn't been generated; CI runs that target
// before `npm test`, so the skip is for the developer who hasn't.
...
// fails silently in the other — extensionless cross-links were once correct in
// the SPA and forty dead links on the repository host.
//
...
//     URL, because that tree has NO markdown source in the repo, so no relative
//     path can resolve there. stripDocsOrigin keeps those navigating in-SPA.
