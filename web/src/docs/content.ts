...
export type DocPage = { path: string; title: string; body: string };
export type NavItem = { text: string; link: string; icon: LucideIcon };
export type NavGroup = { text: string; items: NavItem[] };

// Pull the front-matter `title` (and strip the block) so the body handed to the
// Markdown renderer has no YAML; fall back to the first H1.
...
for (const [key, src] of Object.entries(raw)) {
  const { title, body } = parse(src);
  const path = routeFor(key);
  PAGES[path] = { path, title, body };
}
...
function catalogGroups(): NavItem[] {
  return Object.values(PAGES)
    ...
    .map((p) => ({ text: p.title, link: p.path, icon: Plug }))
