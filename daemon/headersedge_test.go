...
// the port is declared (drops/trigger/webhook_input.go) and carries
// application/json, which is exactly what a Collections "Rows" input takes.
//
// It used to be deleted. A data-model migration dropped every edge whose port
// was NAMED "headers" — matched by name, with no manifest check — and it ran on
// every save AND every load. So the canvas accepted the wire, the autosave
// returned 200, and the edge was gone by the time anything read the flow back:
// the run failed with missing_input on an input the author had just connected,
// and the same fate met http_request's and every Web API step's Headers output.
// Ports are removed from manifests, not filtered by name, so the round-trip
// must carry whatever the port rules accept.
