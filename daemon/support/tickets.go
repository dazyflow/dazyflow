...
// The store never scrubs bodies itself — the route layer does that on ingest —
// and never invents a status the core model rejects.
...
// The twin of the Postgres one, which carries the reasoning.
...
type PgTicketStore struct {
	...
	// An unindexable GROUP BY over every ticket ever filed, so it is cached: ~30ms
	// single-shot at 200k rows, but p95 182ms under 20 concurrent agents.
...
func (s *PgTicketStore) QueueSummary(ctx context.Context) (core.TicketQueueSummary, error) {
	...
	// Exactly ONE recomputes, or every TTL expiry is a stampede.
	...
	gen := s.summaryGen
	s.summaryMu.Unlock()
	// Deferred, or a recovered panic leaves the in-flight flag stuck true.
	defer func() {
		s.summaryMu.Lock()
...
// A thread belongs to the ORG, so it survives one member leaving; their
// identifiers and their own words go. Returns ROWS changed, counting a ticket
// once however many of its columns named the person.
func (s *PgTicketStore) AnonymizeSubject(ctx context.Context, ident string) (int, error) {
	...
	defer func() { _ = tx.Rollback(ctx) }()
	total := 0
	// Bound rather than concatenated: the constant was not injectable, but it was the
	// one place here building SQL by concatenation, and that is what gets copied to a
...
// Open tickets are never pruned: an unanswered one is backlog, not garbage.
