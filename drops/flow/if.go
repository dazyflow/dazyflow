			...
			Examples: []core.ParamsExample{
				{
					...
					Notes:  "Wire the text into A; B is the literal \"urgent\". A leaves via Yes when it contains \"urgent\", otherwise via No.",
				},
				{
	...
	engine.Register(engine.NativeDrop{
		Manifest: core.Manifest{
			...
			ParamsSchema: json.RawMessage(`{
				"type":"object",
				"properties":{
					"B":{"type":"string","title":"Substring","description":"The text to look for inside A, when the B input isn't wired."}
				}
			}`),
			Idempotent:    true,
			NoPassthrough: true,
