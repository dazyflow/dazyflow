...
func init() {
	...
	engine.Register(engine.NativeDrop{
		Manifest: core.Manifest{
			...
			Description: "Wait for N connected inputs to arrive and emit them as a single list on the out port. Useful as a synchronization point in parallel branches.",
			Summary:     "Synchronize parallel branches by collecting every connected input into a single list emitted on out.",
			Examples: []core.ParamsExample{
				{
					Title:  "No configuration — merge takes no params",
					Params: json.RawMessage(`{}`),
					Notes:  "Connect two or more earlier output ports into the variadic items input.",
				},
			},
