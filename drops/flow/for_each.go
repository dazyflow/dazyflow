		...
		Manifest: core.Manifest{
			...
			Examples: []core.ParamsExample{
				...
				},
				{
					...
					Notes:  "Wire the 'Loop body' pin to an HTTP step whose body is ${item.} (the whole item as JSON) or ${item.field.subfield} for a nested value. concurrency caps how many run at once.",
				},
				{
					Title:  "Send a templated email per recipient, stop on first failure",
					Params: json.RawMessage(`{"fail_fast":true}`),
					Notes:  "Wire the 'Loop body' pin to an Email step using ${item.email} / ${item.name}. fail_fast aborts the loop on the first failed item.",
				},
			},
