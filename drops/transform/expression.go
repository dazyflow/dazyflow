...
func init() {
	engine.Register(engine.NativeDrop{
		Manifest: core.Manifest{
			...
			Description: "Compute a single value from a formula — the value-level companion to the calculated-column drop. Write one CEL expression (the same formula language the row tools use) with the wired 'in' value available as `input` and the current time as `now`. Use it to reshape a value mid-flow: pull a field (`input.user.email`), do arithmetic (`input * 1.25`), build a string (`\"Hi \" + input.name`), test a condition (`input.status == \"paid\"`), or transform a list (`input.map(x, x.id)`). The result is emitted on 'out', typed by what the formula returns (text, boolean, or JSON). For running real OS commands or scripts, use the Shell drop instead — this is a safe, sandboxed expression evaluator, not a general code runtime.",
