...
func init() {
	engine.Register(engine.NativeDrop{
		Manifest: core.Manifest{
			...
			Description: "Send an email from the connected mailbox. To, Subject and Body can be typed as params or connected from an earlier step (the matching input port overrides the param) — handy for per-recipient sends. Attach files by connecting file-producing steps (e.g. Export Sheet as PDF) into the variadic 'attachments' input.",
			Integration: "Gmail",
			Category:    "network",
			Icon:        "mail",
			...
			Examples: []core.ParamsExample{
				...
				{Title: "Daily report with a PDF attachment", Params: json.RawMessage(`{"to":"me@example.com","subject":"Yesterday's comments","body":"Comments digest attached.","token":"${secret.GMAIL_OAUTH}"}`), Notes: "Connect a file-producing step (e.g. Export Sheet as PDF) into the variadic 'attachments' input."},
			},
