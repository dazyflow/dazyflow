...
func init() {
	engine.Register(engine.NativeDrop{
		Manifest: core.Manifest{
			...
			Outputs: []core.Port{
				{Port: "refund_id", Label: "Refund ID", MIME: []string{"text/plain"}, Example: json.RawMessage(`"re_3QxPl2Fk9mNaB1cD"`)},
				{Port: "status", Label: "Status", MIME: []string{"text/plain"}, Example: json.RawMessage(`"succeeded"`)},
				{Port: "meta", Label: "Details", MIME: []string{"application/json"},
					Example: json.RawMessage(`{"id":"re_3QxPl2Fk9mNaB1cD","status":"succeeded","amount":24900,"payment_intent":"pi_3QxPkzFk9mNaB1cD"}`)},
			},
			ParamsSchema: json.RawMessage(`{
				...
				"properties":{
					...
					"amount":{"type":"integer","title":"Amount (cents/öre)","minimum":1,"description":"Leave empty to refund the whole payment. For a partial refund, enter the amount in the currency's smallest unit — e.g. 500 = 5.00 USD/EUR (cents), but 500 = 500 JPY (yen has no smaller unit). Overridden by the 'Amount' input when connected."},
