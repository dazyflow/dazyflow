			...
			Examples: []core.ParamsExample{
				{
					...
					Notes:  "Wire 'Plan' and 'Customer' into a notify step for a churn alert, or into Search customers → email for a win-back message.",
				},
			},
...
func executeStripeOnSubscriptionCanceled(_ context.Context, job core.Job, _ chan<- core.Progress) (core.Result, error) {
	return core.Result{
		...
		Error: &core.JobError{
			...
			Message: "This trigger only fires when a real customer.subscription.deleted webhook arrives. To test it, cancel a test-mode subscription in the Stripe dashboard; running the graph manually leaves the trigger with no event to feed downstream.",
