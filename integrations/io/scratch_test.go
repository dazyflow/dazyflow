...
// TestScratchScheme_ExcelRoundTrip proves the scratch:// scheme works for
// the excel drops too (not just file_write/read) — excel_write lands the
// workbook in scratch, excel_read resolves the same scheme back.
func TestScratchScheme_ExcelRoundTrip(t *testing.T) {
	workspace := t.TempDir()
	scratch := t.TempDir()

	wres, err := executeExcelWrite(t.Context(), core.Job{
		WorkspaceRoot: workspace,
		ScratchRoot:   scratch,
		Params:        map[string]any{"path": "scratch://report.xlsx"},
		Input: map[string]core.Ref{
			"rows":    {Inline: []map[string]any{{"name": "Alice", "age": 30}}},
			"headers": {Inline: []string{"name", "age"}},
		},
	}, nil)
	if err != nil || wres.Status != core.StatusOK {
		t.Fatalf("excel_write: status=%q err=%v (%+v)", wres.Status, err, wres.Error)
	}
	if got := wres.Output["out"].Ref; got != "scratch://report.xlsx" {
		t.Errorf("output Ref = %q, want scratch://report.xlsx", got)
	}
	if _, err := os.Stat(filepath.Join(scratch, "report.xlsx")); err != nil {
		t.Errorf("workbook not under scratch root: %v", err)
	}

	rres, err := executeExcelRead(t.Context(), core.Job{
		WorkspaceRoot: workspace,
		ScratchRoot:   scratch,
		Params:        map[string]any{"path": "scratch://report.xlsx"},
	}, nil)
	if err != nil || rres.Status != core.StatusOK {
		t.Fatalf("excel_read: status=%q err=%v (%+v)", rres.Status, err, rres.Error)
	}
	rows, ok := rres.Output["rows"].Inline.([]map[string]string)
	if !ok || len(rows) != 1 || rows[0]["name"] != "Alice" {
		t.Errorf("excel_read rows = %#v, want one Alice row", rres.Output["rows"].Inline)
	}
}
